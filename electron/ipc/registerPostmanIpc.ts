...
export function registerPostmanIpc(): void {
  ...
  // 创建项目
  ipcMain.handle("postman:createProject", (_event, input: Parameters<typeof postmanService.createProject>[0]) => {
    return postmanService.createProject(input);
  });

  // 更新项目
  ipcMain.handle("postman:updateProject", (_event, id: number, input: Parameters<typeof postmanService.updateProject>[1]) => {
  ...
  // 获取项目的所有分组
  ipcMain.handle("postman:getGroupsByProjectId", (_event, projectId: number) => {
    return postmanService.getGroupsByProjectId(projectId);
  });

  // 创建分组
  ipcMain.handle("postman:createGroup", (_event, input: Parameters<typeof postmanService.createGroup>[0]) => {
    return postmanService.createGroup(input);
  });

  // 更新分组
  ipcMain.handle("postman:updateGroup", (_event, id: number, input: Parameters<typeof postmanService.updateGroup>[1]) => {
  ...
  // 获取项目的所有请求
  ipcMain.handle("postman:getRequestsByProjectId", (_event, projectId: number) => {
  ...
  // 创建请求
  ipcMain.handle("postman:createRequest", (_event, input: Parameters<typeof postmanService.createRequest>[0]) => {
    return postmanService.createRequest(input);
  });

  // 更新请求
  ipcMain.handle("postman:updateRequest", (_event, id: number, input: Parameters<typeof postmanService.updateRequest>[1]) => {
  ...
  // 批量创建请求
  ipcMain.handle("postman:batchCreateRequests", (_event, requests: Parameters<typeof postmanService.batchCreateRequests>[0]) => {
  ...
  // 添加历史记录
  ipcMain.handle("postman:addHistory", (_event, input: Parameters<typeof postmanService.addHistory>[0]) => {
  ...
  // 保存设置
  ipcMain.handle("postman:saveSetting", (_event, key: string, value: Record<string, unknown>) => {
