            ...
            if len(section_content) > self._chunk_size:
                ...
                for text_chunk in text_chunks:
                    chunks.append(NoteChunk(
                        ...
                        file_name=file_name,
                        heading=heading,
                        content=text_chunk.content if hasattr(
                            text_chunk, 'content') else text_chunk,
...
class NotesVectorStore:
    ...
    async def _ensure_collection(self):
        ...
        if self._vectorstore.collection_exists(NOTES_COLLECTION_ID):
            # 检查现有集合的维度是否匹配
            existing_dim = self._vectorstore.get_collection_dimension(
                NOTES_COLLECTION_ID)
        ...
        if not self._vectorstore.collection_exists(NOTES_COLLECTION_ID):
            ...
            logger.info(
                f"[NotesVectorStore] 创建笔记向量集合: {NOTES_COLLECTION_ID}, 维度={expected_dim}")
    ...
    async def index_note(
        ...
        try:
            ...
            # 分块
            chunks = self._markdown_splitter.split(
    ...
    async def delete_note(self, file_path: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                df = table.to_pandas()
                matching_ids = df[df['metadata'].str.contains(
                    f'"file_path": "{file_path}"')]['id'].tolist()

                if matching_ids:
                    # 删除匹配的记录
                    for doc_id in matching_ids:
                        self._vectorstore.delete_document(
                            NOTES_COLLECTION_ID, doc_id)

                    logger.debug(
                        f"[NotesVectorStore] 删除笔记向量: {file_path}, {len(matching_ids)} 条")
