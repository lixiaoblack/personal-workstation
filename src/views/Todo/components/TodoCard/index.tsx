...
export const TodoCard: React.FC<TodoCardProps> = ({ category, onTaskToggle }) => {
    ...
    <div className="rounded-xl border border-border bg-bg-secondary/70 p-6 backdrop-blur-sm">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`size-10 rounded-lg ${colorConfig.bg} flex items-center justify-center ${colorConfig.text}`}>
            <span className="material-symbols-outlined">{category.icon}</span>
          </div>
          <h4 className="text-lg font-bold text-text-primary">{category.label}</h4>
        </div>
        <span className={`rounded ${colorConfig.bg} ${colorConfig.text} px-2 py-1 text-xs font-bold`}>
          {pendingCount} 个待办
        </span>
      </div>
        ...
        {category.tasks.map((task: Task) => (
          <div key={task.id} className="flex items-start gap-3 group">
            ...
            <div className="flex-1">
              ...
              <span className={`mt-1 flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider ${
                task.status === "overdue" ? "text-error" : "text-text-tertiary"
              }`}>
                {task.status === "overdue" && (
