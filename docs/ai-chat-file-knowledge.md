...
当用户拖拽文件到聊天框并发送后，Agent 会先分析文件内容，然后询问用户是否需要将文件添加到知识库。

```
用户拖拽文件 → Agent 分析文件 → 发送 knowledge_ask_add 消息
                                           ↓
                        前端显示 KnowledgeSelectCard
                                           ↓
                        用户选择知识库 → 调用 addKnowledgeDocument API
                                           ↓
                        显示添加结果（成功/失败）
```
...
#### knowledge_ask_add 消息

**类型定义**：[electron/types/websocket.ts](file:///Users/wanglixiao/Desktop/个人/personal-workstation/electron/types/websocket.ts)
...
```python
# Agent 执行完成后，如果有附件，发送 knowledge_ask_add 消息
if attachments:
    for att in attachments:
        await self.send_callback({
            ...
            "content": f"已分析文件「{att.get('name')}」，是否需要将其添加到知识库？",
            "attachment": {
                "id": str(uuid.uuid4()),
                "name": att.get("name"),
                "path": att.get("path"),
...
**文件**：[src/views/AIChat/index.tsx](file:///Users/wanglixiao/Desktop/个人/personal-workstation/src/views/AIChat/index.tsx)
...
```typescript
const handleSelectKnowledgeForAdd = async (attachmentId: string, knowledgeId: string) => {
  const state = knowledgeAskAddState[attachmentId];
  
  // 更新状态为处理中
  setKnowledgeAskAddState((prev) => ({
    ...prev,
    [attachmentId]: { ...prev[attachmentId], processing: true },
  }));
  ...
  setKnowledgeAskAddState((prev) => ({
    ...
    [attachmentId]: {
      ...
      addResult: result.success
        ? { success: true, documentName: result.document?.fileName, chunkCount: result.document?.chunkCount }
...
┌─────────────────────────────────────────────────────────────────────┐
│                    Python Agent 分析文件内容                         │
│              (使用 file_read 工具读取并分析)                         │
└──────────────────────────────┬──────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────┐
│                 发送 chat_stream_end 消息                           │
│                 发送 knowledge_ask_add 消息                         │
└──────────────────────────────┬──────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────┐
│             前端显示 KnowledgeSelectCard 组件                        │
│   ┌─────────────────────────────────────────────────────────┐      │
│   │  📄 文件名.pdf                                          │      │
│   │  [添加到知识库]  [暂不需要]                              │      │
│   └─────────────────────────────────────────────────────────┘      │
