...
function registerIpcHandlers() {
  ...
  ipcMain.handle("ocr:recognize", async (_event, imageBase64: string) => {
    try {
      ...
      const response = await post<{ success: boolean; text: string; blocks: Array<{ text: string; confidence: number; box: number[][] }>; error?: string }>(
        "/api/ocr/recognize",
  ...
  // OCR 识别图片文件
  ipcMain.handle("ocr:recognizeFile", async (_event, filePath: string) => {
    try {
      // 需要传递 filePath 作为查询参数
      const { get } = await import("./services/pythonApiClient");
      const response = await get<{ success: boolean; text: string; blocks: Array<{ text: string; confidence: number; box: number[][] }>; error?: string }>(
  ...
  ipcMain.handle(
    "ocr:saveToKnowledge",
    async (_event, knowledgeId: string, title: string, content: string) => {
      try {
        const { post } = await import("./services/pythonApiClient");
