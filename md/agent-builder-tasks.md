...
- [ ] M3-01: 数据库 - agent_conversations 表
...
- [ ] M4-03: Electron IPC - 模板通信
  - 扩展 `electron/ipc/registerAgentBuilderIpc.ts`
  - 模板相关 IPC 处理器

- [ ] M4-04: 组件 - AgentTemplateModal 模板选择弹窗
...
- [ ] M5-01: 数据库 - workflows 表
  - 创建 `workflows` 表
  - 字段：id, agent_id, name, nodes, edges, variables, created_at, updated_at

- [ ] M5-02: 后端 API - 工作流 CRUD
...
- [ ] M5-06: 节点组件开发
  - 创建 `src/views/Workflow/components/nodes/`
  - StartNode 开始节点
  - EndNode 结束节点
  - LLMNode LLM 调用节点
  - ToolNode 工具调用节点
...
- [ ] M5-07: 节点面板与配置面板
  - 创建 `src/views/Workflow/components/NodePanel/`
  - 创建 `src/views/Workflow/components/NodeConfig/`
  - 节点拖拽到画布
  - 点击节点显示配置
...
1. **M1** → 完成后可看到智能体列表，基础框架搭建完成
2. **M2** → 完成后可创建智能体，核心功能可用
3. **M3** → 完成后可与智能体对话，完整闭环
4. **M4** → 完成后降低使用门槛
5. **M5** → 高级功能，按需开发
