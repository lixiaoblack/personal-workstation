...
- **AI 创建待办分类确认流程修复**
  - 即使没有分类也强制调用 `ask_todo_category` 让用户确认
  - 无分类时弹出 UI 显示“暂无分类”提示，用户可选择“不需要分类”或“取消”
  - 添加“取消”选项，用户可以取消创建待办
  - 更新工具描述，强调分类确认的强制流程

- **新增创建待办分类工具**
  - 添加 `create_todo_category` 工具，支持 AI 直接创建分类
  - 区分两种场景：用户明确要求新建分类 vs 用户未指定分类
  - 用户说"新建一个分类"时，AI 自动创建分类，不再询问

- **AskCard 支持输入新分类**
  - 选择分类界面添加"新建分类"选项
  - 用户可以输入新分类名称，AI 自动创建并使用
  - 无需预先创建分类，一站式完成

- **修复附件文件处理问题**
  - 用户上传文件后，直接读取文件内容注入消息
  - 解决模型不调用 file_read 工具的问题
  - 文件大小限制 100KB，超过则提示使用工具

- **AI 创建待办自动设置时间**
  - 工具描述强调必须设置截止时间
  - 未指定时间时自动设置默认值（今天/明天 18:00）
  - 提醒时间自动设置为截止时间前 1 小时
...
- 修复 Windows 平台托盘图标和窗口图标显示错误
  - 添加多尺寸图标文件（16/32/48/64/128/256px）
  - 托盘图标使用 32px 小尺寸图标并正确调整大小
  - 主窗口添加 icon 配置用于显示左上角图标
  - 将图标资源添加到 extraResources 确保打包后可访问
