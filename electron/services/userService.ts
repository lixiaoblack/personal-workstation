...
export function login(credentials: LoginCredentials): LoginResult {
  ...
  // 查找用户
  const user = db.prepare('SELECT * FROM users WHERE username = ?').get(credentials.username) as (User & { password_hash: string }) | undefined;
 ...
 * 更新用户资料
 */
export function updateProfile(userId: number, data: UpdateProfileData): User | null {
  const db = getDatabase();
 ...
 * 更新密码
 */
export function updatePassword(userId: number, data: UpdatePasswordData): { success: boolean; error?: string } {
  const db = getDatabase();
  ...
  try {
    db.prepare('UPDATE users SET password_hash = ?, updated_at = datetime("now", "localtime") WHERE id = ?').run(newPasswordHash, userId);
 ...
 * 重置密码（忘记密码场景）
 */
export function resetPassword(data: ResetPasswordData): { success: boolean; error?: string } {
  const db = getDatabase();
  
  // 检查用户是否存在
  const user = db.prepare('SELECT id FROM users WHERE username = ?').get(data.username) as { id: number } | undefined;
  ...
  try {
    db.prepare('UPDATE users SET password_hash = ?, updated_at = datetime("now", "localtime") WHERE id = ?').run(newPasswordHash, user.id);
